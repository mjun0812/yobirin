...
## 16 インストール直後の確認用通知 (Requirement 20)

使い捨てプロファイル `verify` を新規作成して確認し、検証後に `uninstall --profile verify` で撤去した。

- [x] 新規インストールで確認用の通知が発行される (`ps` に `Installation complete` / timeout=30 の待機プロセスを確認。2026-07-28)
- [x] 応答を待たずにインストールが完了する (`install` の所要時間 0.61秒 / 再検証 0.79秒。通知の30秒タイムアウトを待っていない)
- [x] 上書きインストールでは発行しない (待機プロセスが増えず、完了出力にも案内が出ない)
- [x] 発行したときだけ完了出力に案内が出る (`Sent a confirmation notification. If it does not appear, check System Settings > Notifications.`)
- [x] 未導入プロファイルの初回インストールで通知許可ダイアログが表示され、許可後に当該バンドルの名義で通知が配信される (ユーザー参加で実施)
- [x] 応答しなければ通知センターに残らない (許可済み状態で再検証。5秒=バナー表示 → 15秒=通知センターに1件 → 33秒=プロセス終了 → 38秒=通知センターから消滅。一切触れずにタイムアウト経路で消えることを確認)
- [x] 許可ダイアログが未応答の間はタイムアウトが進まない (design.mdに既知のエッジとして記載した挙動を実測で再確認。41秒経過時点でも待機継続、許可直後にタイマー開始)
