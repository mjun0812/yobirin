...
- **In scope**: yobirin CLI本体 (通知送信・応答捕捉・結果JSON出力・終了コード)、CLI自身によるバンドル組み立てとインストール (`~/Applications` 配置 + PATH上のsymlink)、アイコンプロファイル (派生バンドル)、インストール済みバンドルの一覧表示、待機中プロセスの一覧表示、実行ファイルのビルドパイプライン、ビルド済みバイナリの配布
