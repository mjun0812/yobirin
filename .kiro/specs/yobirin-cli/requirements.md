...
### Requirement 19: バンドルのアンインストール

**Objective:** 導入したyobirinを整理したい利用者として、`install` が作ったものだけを対称的に取り除きたい。それにより、パッケージマネージャが管理するコマンドを壊さずに、バンドルとその登録を確実に消せる。

#### Acceptance Criteria

1. When アンインストールが要求された場合、yobirin は対象バンドル (`--profile` 指定時はそのプロファイル、省略時はデフォルト) を削除しなければならない
2. When バンドルを削除する場合、yobirin は削除に先立ってそのバンドルのLaunchServices登録を解除しなければならない (登録だけが残ると同一Bundle IDの重複登録となり、別コピーの誤起動を招くため)
3. yobirin は PATH上のコマンド (`install` が張るsymlink、およびパッケージマネージャが配置した実バイナリ) を削除してはならない
4. If 対象バンドルが存在しない場合、yobirin はその旨をstderrへ出力し、非0の終了コードで終了しなければならない
5. プロファイル名の検証は `install` と同一の規約に従わなければならない
6. アンインストールは通知APIに触れず、`.app` バンドル外の素のバイナリからも完走しなければならない
7. When 削除したバンドルを指すsymlinkがPATH上の規定位置に残る場合、yobirin はその存在を案内しなければならない (削除はしない)。案内の有無は成否・終了コードに影響しない
8. If LaunchServices登録の解除に失敗した場合、yobirin はその旨をstderrへ出力した上でバンドルの削除を継続しなければならない

### Requirement 20: インストール直後の通知確認

**Objective:** yobirinを導入した利用者として、実際に通知が必要になる前に通知が届く状態かを確かめたい。それにより、肝心の場面で通知が黙って届かない事態を避けられる。

#### Acceptance Criteria

1. When 新規のバンドル (デフォルト・プロファイルを問わず) のインストールが完了した場合、yobirin はそのバンドルの名義で確認用の通知を1件配信しなければならない
2. When 確認用の通知を配信する場合、yobirin はその応答を待たずにインストールを完了しなければならない (通知許可ダイアログの表示中はタイムアウトが進まないため、応答を待つとインストールが長時間返らなくなりうる)
3. 確認用の通知の配信可否および応答内容は、インストールの成否と終了コードを変えてはならない
4. If インストールが既存バンドルの置き換えである場合、yobirin は確認用の通知を配信してはならない (通知許可は初回のインストールで確定するため、再インストールのたびに通知が増えるのを避ける)
5. When 確認用の通知を配信した場合、yobirin は通知が表示されなかった場合の確認先を完了出力に案内しなければならない
6. 確認用の通知は、応答がないまま通知センターに残り続けてはならない
