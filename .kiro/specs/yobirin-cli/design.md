...
- プロファイルの選択機構は**プロファイルごとのsymlink** (`<BIN_DIR>/yobirin-<name>` → 各バンドルのMach-O) に確定 (2026-07-26実装時決定)。`--profile` ディスパッチはSwift側が他バンドルのパスへ結合するため不採用。ビルドは `build-app.sh <profile>`、インストールは `install.sh <profile>...` (プロファイル名は `^[a-z0-9]+$` に制限)
