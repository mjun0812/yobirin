...
- [x] 1. Foundation: Swift Packageとビルド基盤
...
- [x] 4. Integration: 配線・インストール・プロファイル
...
- [x] 7. Core: プロファイルディスパッチとインストーラ部品
- [x] 7.1 (P) プロファイル規約とディスパッチを実装する
...
- [x] 8.2 素のバイナリからのブートストラップを実機で確認する
...
- [x] 9.2 手動検証チェックリストを追補して実施する
  - manual-verification.md にブートストラップ・バンドル外安全性・`--profile` 配信の項目を追加して実施する
  - 全項目の実施結果が記録されている
  - _Requirements: 10.2, 10.3, 12.2, 12.3, 13.2_

- [x] 10. Core: インストール済みバンドルの一覧表示
- [x] 10.1 プロファイル規約の逆引きと一覧生成を実装する
  - バンドル名からプロファイル名への逆引きを規約の単一ソースへ追加する (往復一致で規約外の名前を棄却する)
  - 配置ディレクトリを走査し、規約に一致するバンドルだけを対象にする (無関係なアプリを含めない)
  - 各項目のBundle IDとバージョンを配置済みバンドルの実態から読み取り、読めない項目は欠損として残りの一覧を継続する
  - デフォルト項目はプロファイルと区別できる表示 (テキストは `(default)`、JSONは `profile: null`) で一覧へ含める
  - 一覧はデフォルト先頭・プロファイル名昇順の決定的な順序で、テキストとJSONの両形式を生成できる
  - 0件 (配置ディレクトリ不存在を含む) は正常終了、走査失敗は環境エラーとして扱う
  - 通知APIの型に一切触れない
  - テンポラリ領域に構成した偽バンドルで、対象判定 (採用と棄却)・デフォルトの区別表示・順序・欠損表示・0件の正常終了・走査失敗の非0終了・JSONスキーマがテストで確認できる
  - _Requirements: 14.1, 14.2, 14.3, 14.4, 14.5, 14.6, 14.7, 14.8, 14.9_
  - _Boundary: ListCommand, ProfileDispatch (ProfileNaming)_

- [x] 10.2 listサブコマンドを結線して確認する
  - ルートコマンドへ `list` を登録し、起動ゲートのバンドル外許可リストへ追加する
  - バンドル外の実プロセスで `list` / `list --json` が通知APIに触れず完走することが結合テストで確認できる
  - 実機の `yobirin list` がインストール済みバンドル (デフォルト含む) を表示し、`swift test` で全テストが通る
  - _Requirements: 14.1, 14.5, 14.10_
  - _Depends: 10.1_

- [x] 11. Core: 待機中プロセスの一覧表示
- [x] 11.1 プロセス走査と待機一覧の生成を実装する
  - 自ユーザーのプロセスを列挙し、実行ファイルパス・argv・起動時刻を読み取る走査部品を実装する (走査・現在時刻はfake注入可能)
  - 対象判定は「命名規約のバンドル内Mach-Oを実行パスに持つ」かつ「argvに `--title` または `--title=` を含む」のANDとし、install / list / ps / 掃除 (引数なし) と自分自身のPIDを含めない
  - 各項目のプロファイル (実行パスから逆引き。デフォルトは区別できる表示)・タイトル・タイムアウト指定 (`--title x` / `--title=x` 両形式の抽出)・経過時間を導出する
  - argvが読み取れないプロセスは欠損として表示し、残りの一覧を継続する
  - 一覧は起動時刻の古い順 (同時刻はPID昇順) で、テキスト (経過時間は人間可読に整形) とJSON (psJSON契約) の両形式を生成できる
  - 0件は正常終了、PID列挙の失敗は環境エラーとして扱う
  - 通知APIの型に一切触れない
  - fakeレコード注入で、対象判定 (採用と除外の両方)・デフォルトの区別表示・タイトル/タイムアウト抽出・順序・欠損表示・0件の正常終了・走査失敗の非0終了・経過時間整形・JSONスキーマがテストで確認できる
  - _Requirements: 15.1, 15.2, 15.3, 15.4, 15.5, 15.6, 15.7, 15.8_
  - _Boundary: PsCommand, ProfileDispatch (ProfileNaming)_

- [x] 11.2 psサブコマンドを結線して確認する
  - ルートコマンドへ `ps` を登録し、起動ゲートのバンドル外許可リストへ追加する
  - バンドル外の実プロセスで `ps` / `ps --json` が通知APIに触れず完走することが結合テストで確認できる
  - 実機で待機中の通知が存在する状態の `yobirin ps` に当該プロセス (PID・タイトル) が表示され、`swift test` で全テストが通る
  - _Requirements: 15.1, 15.5, 15.9_
  - _Depends: 11.1_

- [x] 12. アイコン変更時の反映遅延の案内を実装する
  - インストーラが旧バンドル削除の前に既存アイコンを読み取り、「既存バンドルを置き換えたか」「アイコンが変化したか」を結果として呼び出し元へ返す (旧アイコンが読み取れない場合は変化扱い)
  - installサブコマンドの完了出力を、ListCommandと同様に注入可能な出力経路 (stdoutWriter相当) 経由に改め、案内文言の有無をテストから直接検証できるようにする
  - installサブコマンドは「置き換え かつ アイコン変化」のときだけ、完了メッセージに続けて反映遅延の案内 (ログアウト→ログインが必要 / 新しいプロファイル名なら即時反映) を表示する。新規インストールと不変の上書きでは表示しない
  - インストーラの戻り値型変更とstdoutWriter注入に伴い、既存のInstallCommandTestsのfakeクロージャとperform呼び出し箇所を新シグネチャへ追従させる
  - 案内の有無によってインストールの成否・終了コードが変化しない
  - テンポラリ領域のテストで、新規 / 不変上書き / 変化上書き / 旧アイコン読み取り不能の4分岐の案内有無と、終了コード不変が確認できる
...
- 13.2 (2026-07-28、CI失敗から判明): 結合テストで**Apple署名済みバイナリ (`/bin/echo` 等) を複製したスタブは、ad-hoc再署名してもGitHub Actions macOSランナーでSIGKILL (exit 9) される** (ローカルmacOSでは通るため見逃しやすい)。実行可能スタブが要るときは shebangスクリプト (`#!/bin/sh`) にする — 署名不要でexecvがそのまま解釈し、環境に依存しない
