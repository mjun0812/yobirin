...
### Decision: バンドル外の通知系要求は、インストール済みデフォルトバンドルへexecで引き継ぐ (2026-07-28)

- **Context**: Requirement 17。miseの実測 (ubi / github両backendで取得可能) により、パッケージマネージャが素のバイナリをPATHへ置く導線が現実になったが、素のバイナリからは通知が出せず、`~/.local/bin/yobirin` とのPATH二重化で通知が案内エラーになる
- **Selected Approach**: 起動ゲートに「バンドル外×通知系/引数なし×デフォルトバンドル有」の分岐を追加し、`--profile` ディスパッチと同じexecv機構でバンドル内Mach-Oへ引き継ぐ。バージョン不一致はstderr案内のみで続行。あわせて `ProfileNaming` のホーム解決へ `YOBIRIN_HOME` 上書きを導入し、結合テストを密閉する
- **Rationale**: 既存の不変条件 (execvでargv[0]差し替え・引き継ぎ先はバンドル内実行) の再利用で、mise shim・ダウンロード直後のバイナリ・`.build` 成果物のどれからでも透過的に通知が動く。自動installのような暗黙の副作用は入れない (副作用の分離原則を維持)
- **Trade-offs**: 素のバイナリの通知要求が「案内して終了」から「引き継いで配信」へ変わる (Requirement 12.2/12.3は未インストール時のfallbackへ格下げ)。バージョン不一致時に古いバンドルで配信される可能性があるが、案内で更新を促すに留める (暗黙更新はしない)

### Decision: `uninstall` はバンドルと登録のみを消し、PATH上のコマンドには触れない (2026-07-28)

- **Context**: Requirement 19。手で `rm -rf` するとLaunchServices登録だけが残り、同一Bundle IDの重複登録になる。実測でも `com.mjun0812.yobirin.claude` が正規の `~/Applications` と過去の作業ディレクトリの2箇所に登録され、yobirin系で計18件の登録が残っていた
- **Selected Approach**: 対象バンドルの登録を `lsregister -u` で解除してから削除する。PATH上のコマンドは削除せず、削除したバンドルを指したまま残るsymlinkは案内するだけに留める
- **Rationale**: mise導入時は `command -v yobirin` がmise管理の実バイナリを指す (実測)。PATH上を消しに行くとパッケージマネージャの管理状態を壊す。バンドルだけが消えた状態は「バイナリはあるがバンドル未インストール」であり、Requirement 12.2/17.3が既に定義済みの状態へ戻るだけで済む
- **Trade-offs**: 手動インストールではバイナリ実体がバンドル内にあるため、削除後は壊れたsymlinkが残る。削除はせず案内する方針としたため、利用者が自分で消すか再インストールする必要がある

### Decision: 新規インストール時に確認用の通知を1件出す。待たない・成否に影響させない (2026-07-28)

- **Context**: Requirement 20。通知許可のダイアログが出るのは「最初に本当に通知を出したとき」であり、それがhook経由の無人時だと、見ていない場所でダイアログが出て肝心の通知が届かない
- **Selected Approach**: インストール完了後、新規インストールに限り、配置したバンドルのMach-Oを通知引数付きで起動する。応答は待たず、起動できなくても終了コードを変えない。新規判定はRequirement 16のアイコン比較で既に持っている「既存バンドルを置き換えたか」を再利用する
- **Rationale**: 起動検証で既にバンドルのMach-Oを実行しているため、機構を増やさずに済む。通知APIをインストーラへ持ち込まない構造 (Requirement 12.1) も、子プロセスがバンドル内実行になることで保たれる。新規のみに限れば `mise up` のたびに通知が増えることもない
- **Trade-offs**: 待たないため配信可否の検証はできない (人が見るかどうかに委ねる)。初回の許可ダイアログを未応答で放置すると、確認用の子プロセスは許可確定まで待ち続ける (`--timeout 5` の通知が48秒待った実測と同根の性質) — 初回通知に共通する既存の挙動であり、`yobirin ps` で可視化できる。同期的に待つ案は、ダイアログ次第でインストールが数分返らなくなるうえ、CIやpostinstallでの無人実行を壊すため採らない
