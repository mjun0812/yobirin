...
### 旧APIで自分の許可済みIDを名乗れるか (2026-07-28実測)

「なりすまし先を他社アプリではなく、UN APIで許可取得済みの自分のID (`com.mjun0812.yobirin`) にすれば、倫理・堅牢性の問題なしに `_identityImage` の通知ごとアイコン (透過込み、1 Bundle IDで複数アイコン) が得られるのでは」という両API併用案を、alerterの `--sender` (swizzleの返すIDを任意指定) で検証した。

| 条件                                                              | 結果                                                                                                                         |
| ----------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `--sender com.mjun0812.yobirin` + `--app-icon` (自分の許可済みID) | ✕ **配信されない**。alerterは配信待ちポーリングのままハング (`--timeout 5` も効かない — timeoutは配信後基準)。強制終了が必要 |
| 既定 (`com.apple.Terminal` 偽装) + `--app-icon` (対照)            | ○ 配信され、指定PNGへの差し替えを目視確認。`@TIMEOUT` で正常終了                                                             |

**結論**: `-sender` 偽装が通るのはシステムアプリ名義のみで、UN APIで許可を得た自分のIDを旧APIから名乗っても**配信そのものが拒否される**。2026-07-26の「正直な `.app` + 旧API + `_identityImage` → 差し替わらない」と合わせ、「透過アイコン + 1 Bundle IDで複数アイコン」という旧APIの利得は**Terminalなりすましの継続と等価**であり、自分名義では成立しない。両API併用案は棄却し、既存の決定 (新API + プロファイル派生バンドル) を維持する。

なお [PR #65](https://github.com/vjeantet/alerter/pull/65) (autoreleaseプールdrainによるリーク修正) はマージされれば「放置でメモリが増え続ける」問題自体は解消する見込みだが、ポーリングの構造 (毎秒約5回のCPU起床、バナーを閉じた瞬間を検知できない却下検知) とdeprecated API・なりすまし依存は残る。
